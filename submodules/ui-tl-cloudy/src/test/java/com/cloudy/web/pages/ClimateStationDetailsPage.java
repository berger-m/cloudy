...
public class ClimateStationDetailsPage extends AbstractClimatePage {
	...
	private void init() {
		if (isNull(tableDetailsColumnStationName) || isNull(tableDetailsColumnProvince)
				|| isNull(tableDetailsColumnDate) || isNull(tableDetailsColumnMeanTemp)
				|| isNull(tableDetailsColumnHmmt) || isNull(tableDetailsColumnLmmt) //
				|| tableDetailsColumnStationName.isEmpty() || tableDetailsColumnProvince.isEmpty()
				|| tableDetailsColumnDate.isEmpty() || tableDetailsColumnMeanTemp.isEmpty()
				|| tableDetailsColumnHmmt.isEmpty() || tableDetailsColumnLmmt.isEmpty()) {
		...
		for (int i = 0; i < tableRows; i++) {
			final WebElement[] row = new WebElement[] { tableDetailsColumnStationName.get(i),
					tableDetailsColumnProvince.get(i), tableDetailsColumnDate.get(i), tableDetailsColumnMeanTemp.get(i),
					tableDetailsColumnHmmt.get(i), tableDetailsColumnLmmt.get(i) };
			tableDetails[i] = row;
		}
	}
