...
public class ClimateSummaryPage extends AbstractClimatePage {
	...
	private void init () {		
		if ( isNull(tableSummaryColumnStationName) || isNull(tableSummaryColumnDate) || isNull(tableSummaryColumnMeanTemp) //
				|| tableSummaryColumnStationName.isEmpty() || tableSummaryColumnDate.isEmpty() || tableSummaryColumnMeanTemp.isEmpty() ) {
		...
		for ( int i = 0; i < tableRows; i++ ) {
			final WebElement[] row = new WebElement[] {tableSummaryColumnStationName.get(i), tableSummaryColumnDate.get(i), tableSummaryColumnMeanTemp.get(i)};
			tableSummary[i] = row;
		}		
	}
